...
import type { HazardSeverity, HazardType } from "../../shared/types";
import type { CctvDetectionResult } from "../../shared/cctvDetection";

const inputSize = 640;
const hazardWords = ["fire", "flame", "smoke", "gas", "haze"];
const defaultLabels = ["fire", "smoke", "gas", "hazard"];

let sessionPromise: Promise<ort.InferenceSession> | undefined;

ort.env.wasm.wasmPaths = "https://cdn.jsdelivr.net/npm/onnxruntime-web@1.24.3/dist/";

export async function loadYoloHazardModel(modelFile: File): Promise<void> {
...
export async function loadYoloHazardModelFromUrl(url: string, onProgress?: (percent: number) => void): Promise<void> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to fetch ONNX model (HTTP ${response.status})${body ? `: ${body}` : ""}`);
  }
...
export function hasLoadedYoloHazardModel(): boolean {
  return Boolean(sessionPromise);
}

export async function runYoloHazardDetection(video: HTMLVideoElement, labels = defaultLabels): Promise<CctvDetectionResult> {
  if (!sessionPromise) {
    throw new Error("Upload a YOLO ONNX model before running real video detection.");
  }
  ...
  return {
    ...
    message: detection.isHazard
      ? `${detection.label} detected by uploaded YOLO model at ${Math.round(detection.confidence * 100)}% confidence.`
      : `Uploaded YOLO model did not detect fire, smoke, gas, or abnormal haze above threshold.`
  };
}
...
export function parseYoloOutputForTest(output: { dims: readonly number[]; data: Float32Array }, labels = defaultLabels) {
...
function parseYoloOutput(output: { dims: readonly number[]; data: unknown }, labels: string[]) {
  ...
  let best = {
    ...
    confidence: 0,
    isHazard: false,
    hazardType: undefined as HazardType | undefined,
  ...
  for (const row of rows) {
    ...
    const hazardType = labelToHazardType(label);
    const isHazard = Boolean(hazardType) || hazardWords.some((word) => label.toLowerCase().includes(word));
    if (!isHazard || row.confidence < 0.35 || row.confidence <= best.confidence) {
      continue;
    }
...
function labelToHazardType(label: string): HazardType | undefined {
  const normalized = label.toLowerCase();
  if (normalized.includes("fire") || normalized.includes("flame")) return "fire";
  if (normalized.includes("smoke") || normalized.includes("haze")) return "smoke";
